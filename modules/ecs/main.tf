...
======*/
resource "aws_cloudwatch_log_group" "nodehelloapp" {
  name = "${var.environment}-nodehelloapp"

  tags {
    Environment = "${var.environment}"
    Application = "node-hello-app"
  }
}
...
data "template_file" "web_task" {
  ...
  vars {
    ...
    log_group = "${aws_cloudwatch_log_group.nodehelloapp.name}"
  }
}
...
resource "aws_alb" "alb_nodehelloapp" {
  name            = "${var.environment}-alb-nodehelloapp"
...
resource "aws_alb_listener" "nodeapplistener" {
  load_balancer_arn = "${aws_alb.alb_nodehelloapp.arn}"
  port              = "80"
  protocol          = "HTTP"
...
resource "aws_cloudwatch_metric_alarm" "service_cpu_high" {
  alarm_name          = "${var.environment}_nodeapp_web_cpu_utilization_high"
